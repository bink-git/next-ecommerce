    ...
    <div className="h-[calc(100vh-80px)] overflow-hidden">
        ...
        {slides.map((slide) => (
          ...
          >
            ...
            <div className="h-1/2 xl:h-full xl:w-1/2 flex flex-col items-center justify-center gap-8 2xl:gap-12 text-center">
              ...
              <Link href={slide.url}>
                <button className="roounded-md bg-black text-white py-3 px-4">
                  Shop Now
                </button>
              </Link>
            </div>
      ...
      <div className="absolute m-auto left-1/2 bottom-8 flex gap-4">
        {slides.map((slide, i) => (
          <div
            key={slide.id}
            className={`w-3 h-3 rounded-full ring-1 ring-black flex cursor-pointer items-center justify-center
             ${current === i ? 'scale-150' : ''}`}
